...
pub const COIN_MAGIC: &str = "ITS-COIN/1";
...
impl CoinManifest {
    ...
    pub fn parse_text(text: &str) -> Result<Self> {
        let lines: Vec<&str> = text.lines().collect();
        if lines.first().map(|l| l.trim()) != Some(COIN_MAGIC) {
            return Err(MemError::Coin("missing ITS-COIN/1".into()));
        }
        let mut room_wire_pk = String::new();
