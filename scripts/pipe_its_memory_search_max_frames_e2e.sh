#!/usr/bin/env bash
# M51 — --max-frames 5 excludes spam-room; quiet-room still matches.
set -euo pipefail
...
TMP="${TMPDIR:-/tmp}/its_memory_m51_$$"
mkdir -p "$TMP"
trap 'rm -rf "$TMP"' EXIT
...
echo "== search --max-frames 5 excludes spam-room =="
SEARCH_OUT="$("$COIN" channel search --max-frames 5 --registry "$REG" 2>&1)"
echo "$SEARCH_OUT"
echo "$SEARCH_OUT" | grep -q 'search hits: 1' || { echo "FAIL: expected 1 search hit" >&2; exit 1; }
echo "$SEARCH_OUT" | grep -q "$QUIET_PK" || { echo "FAIL: quiet-room should match" >&2; exit 1; }
echo "$SEARCH_OUT" | grep -q "$SPAM_PK" && { echo "FAIL: spam-room should be excluded by --max-frames 5" >&2; exit 1; }

echo "pipe_its_memory_search_max_frames_e2e.sh: OK (M51)"
